...
while k < 1024
    while l < ToT_FF_size + 1
        if idx(1,1,l) == 0 && functionaliser_T(1,l) == 0
            TEST = TEST + ToT_FF_T_BIG2(k,3,l);
            
        end
        if functionaliser_T(1,l) == 1
            TEST2 = TEST2 + ToT_FF_T_BIG2(k,3,l);
            counter2 = counter2 + 1;
        end
        l = l + 1;
    end
    AverageTest2(k,1) = TEST;
    AverageTest2_C(k,1) = TEST2;
...
while p < 1024%xsize + 1
    if p == 1
        ...
        AverageTest2_C(p,3) = AverageTest2_C(p,2);
        
    else
        P = p - 1;
        AverageTest2(p,3) = AverageTest2(p,2)+AverageTest2(P,3);
        AverageTest2_C(p,3) = AverageTest2_C(p,2)+AverageTest2_C(P,3);
    end
...
% Cross pixels
AverageTest2_C(:,3) = AverageTest2_C(:,3);
maxy = max(AverageTest2_C(:,3));
AverageTest2_C(:,3) = AverageTest2_C(:,3);

AverageTest2_C(:,4) = AverageTest2_C(:,3)./max(AverageTest2_C(:,3));


%% get curves v2.0 no cross
O = zeros(1024,1);
...
while intercept <  0.99999
    ij = ij + 1;
    O(Avvan:Avtot,1) = 1;
    Try = createFit_poly3(M,AverageTest2_C(:,4), O); %calls funtion createFit,
    % which tries to fit a polynome to the 3th order to the Average
    % Curve for each step
    %AverageCurve(ij,1:4) = [Avvan,Avtot,ZeroFixer(Avvan,1),ZeroFixer(Avtot,1)];
    AverageCurve_C(ij,1:4) = [Avvan,Avtot,AverageTest2(Avvan,4),AverageTest2(Avtot,4)];
    coeffy = coeffvalues(Try); % places the polynomal corves in coeffy
    AverageCurve_C(ij,5:8) = coeffy; % places the coeffy information in the AverageCurve Table
    %intercept = coeffy(1,4) %creates the exit for the while loop. When the fitting curve nears 1
    intercept = (AverageCurve_C(ij,3));
    O(Avvan:Avtot,1) = 0;
    Avvan = Avtot; % cretes a new step start
    Avtot = Avvan + stepAvg; % creates a new step stop
end
...
%%
clearvars xsize x A k l p P strength TEST TEST2 x  p1 p2 p3 maxy coeffy ij lowerB lowerBm upperB upperBm meanAllCounts stdAllCounts stdAllCountsF medianAllCounts
